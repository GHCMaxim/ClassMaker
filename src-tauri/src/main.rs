...
fn parse_file(state: tauri::State<'_, SharedState>, path: String) -> Result<String, String> {
    ...
    *shared_state = Some(Excel::open(path).unwrap());
    if let Ok(range) = shared_state.as_mut().unwrap().worksheet_range("Sheet1") {
        ...
        Ok(format!("{} cells, {} non-empty cells", total_cells, non_empty_cells))
    } else {
        Err("Error".to_string())
    }
}
...
#[tauri::command]
fn sort_by_class_id(state: tauri::State<'_, SharedState>, subject_id: String) -> Result<serde_json::value::Value, String> {
    ...
    if let Ok(range) = excel.as_mut().unwrap().worksheet_range("Sheet1"){
        ...
        for row in range.rows(){
            if let DataType::String(value) = &row[4] {
                if value == &subject_id{
                    let s = if let DataType::String(value) = &row[4] {value.to_string()} else {format!("{:?}", row[4])};
                    let class_id = if let DataType::Float(value) = &row[2] {(*value as i32).to_string()} else {format!("{:?}", row[2])};
                    let included_id = if let DataType::Float(value) = &row[3] {(*value as i32).to_string()} else {format!("{:?}", row[3])};
                    let class_title = if let DataType::String(value) = &row[5] {value.to_string()} else {format!("{:?}", row[5])};
                    let credit = if let DataType::String(value) = &row[7] {value.to_string()} else {format!("{:?}", row[7])};
                    let note = if let DataType::String(value) = &row[8] {value.to_string()} else {format!("{:?}", row[8])};
                    let date = if let DataType::Float(value) = &row[10] {(*value as i32).to_string()} else {format!("{:?}", row[10])};
                    let time = if let DataType::String(value) = &row[11] {value.to_string()} else {format!("{:?}", row[11])};
                    let location = if let DataType::String(value) = &row[16] {value.to_string()} else {format!("{:?}", row[16])};
                    let class_type = if let DataType::String(value) = &row[22] {value.to_string()} else {format!("{:?}", row[21])};
                    let validity = if let DataType::String(value) = &row[23] {value.to_string()} else {format!("{:?}", row[23])};
...
fn get_included_class(state: tauri::State<'_, SharedState>, included_id: String) -> Result<ResultData, String>{
    let mut excel = state.excel.lock().unwrap();
    if let Ok(range) = excel.as_mut().unwrap().worksheet_range("Sheet1"){
...
fn check_validity(state: tauri::State<'_, SharedState>, time: Vec<Time>) -> bool{
    ...
    for i in 0..time.len(){
        if available_positions[time[i].day as usize -2].contains(&time[i].start) || available_positions[time[i].day as usize -2].contains(&time[i].end){
            valid = false;
            break;
        }
        ...
        for j in 0..available_positions[time[i].day as usize - 2].len(){
            if available_positions[time[i].day as usize - 2][j] == time[i].start && available_positions[time[i].day as usize - 2][j+1] == time[i].end{
                break;
            }
            valid = false;
            available_positions[time[i].day as usize - 2].remove(j);
            let index = available_positions[time[i].day as usize - 2].iter().position(|x| *x == time[i].end).unwrap();
...
fn main() {
    ...
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![greet, parse_file, sort_by_class_id])
